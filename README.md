...
- 版本：v0.1.12（SillyTavern 1.18+，兼容 TauriTavern 移动端）
...
- **快照随手清理**：Prune 图标删除不要的叶子快照；分支编号保持稳定，两步确认防止手滑误删。
- **每个聊天互不干扰**：每个聊天都有自己独立的一棵撤销树，切换聊天或角色卡，面板会自动跟着切换。
- **正文预览**：每个分支显示最后一条正文，太长会自动滚动；预设往正文里塞的状态栏、思维链之类的标签，可以在设置里过滤掉。
- **可见的保留策略**：默认保留全部快照，也可设置数量提醒；插件不会在后台自动删除，清理仍需逐项双重确认。
- **长树性能优化**：分支索引会安全缓存，预览优先取聊天目录摘要，只有需要回退查找正文时才按可见节点惰性读取完整聊天。
...
5. 设置入口在扩展面板的 Floor Anchor 抽屉里：可以调预览长度、过滤 XML 标签，并选择“保留全部”或仅在快照达到指定数量时提醒清理。两种模式都不会自动删除文件。
